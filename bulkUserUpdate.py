...
res2 = requests.get(url + '/api/v1/meta/types/user',
...
FBIMemberid = ''
for name in result1:
    if name['displayName'] == 'FBI Member':
        FBIMemberid = name['id']
    elif name['displayName'] == 'FBI Member Admin':
        FBIMemberAdminid = name['id']
...
apiurl= url + '/api/v1/users?search=type.id eq'+ ' ' +'"'+FBIMemberAdminid+'"'+ ' '+'or type.id eq'+ ' '+ '"'+FBIMemberid+'"'
print(apiurl)
